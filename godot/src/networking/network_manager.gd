...
func authenticate(email: String, password: String, create: bool = false, callback: Callable = Callable()) -> bool:
	var result = await _client.authenticate_email_async(email, password, create)
	if result.is_exception():
		var msg := result.get_exception().message
		push_error("Nakama auth failed: " + msg)
		if callback.is_valid(): callback.call(false, msg)
		return false
	_session = result
	await _open_socket()
...
func authenticate_device(callback: Callable = Callable()) -> void:
	var device_id := OS.get_unique_id()
	if device_id.is_empty(): device_id = "guest_%s" % str(randi())
	var result = await _client.authenticate_device_async(device_id, true, "Guest")
	if result.is_exception():
		if callback.is_valid(): callback.call(false, result.get_exception().message)
		return
	_session = result
	await _open_socket()
	if callback.is_valid(): callback.call(true, "")
